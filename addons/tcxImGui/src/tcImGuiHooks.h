...
inline void ImGuiTestEngineHook_ItemAdd(ImGuiContext* ctx, ImGuiID id, const ImRect& bb, const ImGuiLastItemData* item_data) {
    if (!tcx::imgui_tools::detail::collecting) return;
    ...
    tcx::imgui_tools::WidgetInfo info;
    info.id = id;
    info.rect = bb;
    ...
    auto& cur = tcx::imgui_tools::detail::currentFrame;
    auto& idMap = tcx::imgui_tools::detail::currentIdMap;
...
inline void ImGuiTestEngineHook_ItemInfo(ImGuiContext* ctx, ImGuiID id, const char* label, ImGuiItemStatusFlags flags) {
    ...
    auto& widget = tcx::imgui_tools::detail::currentFrame[it->second];
...
inline const char* ImGuiTestEngine_FindItemDebugLabel(ImGuiContext* ctx, ImGuiID id) {
    ...
    auto it = idMap.find(id);
    if (it != idMap.end()) {
        auto& w = tcx::imgui_tools::detail::currentFrame[it->second];
