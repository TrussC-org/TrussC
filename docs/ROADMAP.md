...
| Kiosk / installation mode (trusscli) | First-class answer to "museum runs an app for 3 months": `trusscli kiosk` (or app-side `InstallationSettings`) = watchdog auto-restart on crash/hang, log rotation, crash-report collection (.ips etc.), scheduled start/stop, and a tiny remote health endpoint (HTTP status: fps, uptime, last error). Every long-running oF installation reinvents this with shell loops; nobody ships it as a framework feature. Mostly trusscli / process-level — near-zero core regression risk. | Medium |
